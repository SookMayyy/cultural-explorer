...
//   • Grade-based account creation; Grade 3+ choose a password, Grade 1-2 get a memorable
//     auto-password shown once at registration and cleared after first login.
//   • Icon-based recovery: two ordered icons from a grid of 12. Grade 1-2 reveal the
//     password; Grade 3+ set a new one.
...
describe('FR7 — Grade 3+ registration & session', () => {
  const NAME = name('Stu');
  const agent = request.agent(app);

  test('registers with a password, opens a session, and returns no secrets', async () => {
    const res = await agent.post('/api/auth/register').send({
      display_name: NAME, grade_group: '3-4', password: 'pass123', icon_key_1: 3, icon_key_2: 7,
    });
    expect(res.status).toBe(201);
    expect(res.body.ok).toBe(true);
    expect(res.body.auto_password).toBeUndefined();   // only Grade 1-2 get one
    await trackByName(NAME, '3-4');
  ...
  test('the session user is safe-shaped (no password hash) with sensible defaults', async () => {
    ...
    expect(me.grade_group).toBe('3-4');
...
describe('FR7 — Grade 1-2 auto-password lifecycle', () => {
  const NAME = name('Kid');
  let autoPw, id;
  ...
  test('registration returns a memorable auto-password (no password supplied)', async () => {
    const agent = request.agent(app);
    const res = await agent.post('/api/auth/register').send({
      display_name: NAME, grade_group: '1-2', icon_key_1: 2, icon_key_2: 6,
    });
    expect(res.status).toBe(201);
    autoPw = res.body.auto_password;
    expect(autoPw).toMatch(/^[a-z]+\d[a-z]+$/);       // e.g. "sun7cat"
    id = await trackByName(NAME, '1-2');
  });
  ...
  test('first login with the auto-password clears it and sets a hash', async () => {
    const login = await request.agent(app).post('/api/auth/login')
      .send({ display_name: NAME, grade_group: '1-2', password: autoPw });
...
describe('FR7 — Login failures', () => {
  ...
  beforeAll(async () => {
    await request.agent(app).post('/api/auth/register').send({
      display_name: NAME, grade_group: '3-4', password: 'rightpw', icon_key_1: 1, icon_key_2: 2,
    });
    await trackByName(NAME, '3-4');
  });
  ...
  test('wrong password → 401', async () => {
    const res = await request(app).post('/api/auth/login')
      .send({ display_name: NAME, grade_group: '3-4', password: 'wrongpw' });
    expect(res.status).toBe(401);
  });

  test('unknown user → 401', async () => {
    const res = await request(app).post('/api/auth/login')
      .send({ display_name: name('Ghost'), grade_group: '3-4', password: 'whatever' });
    expect(res.status).toBe(401);
  });
...
describe('FR7 — Registration validation', () => {
  const base = { grade_group: '3-4', password: 'pass123', icon_key_1: 3, icon_key_2: 7 };
  ...
  test('Grade 3+ without a password → 422', async () => {
...
describe('FR7 — Icon-based recovery', () => {
  test('Grade 3+ recovery with correct icons sets a new password', async () => {
    const NAME = name('Rec');
    await request(app).post('/api/auth/register').send({
      display_name: NAME, grade_group: '3-4', password: 'oldpass', icon_key_1: 5, icon_key_2: 9,
    });
    await trackByName(NAME, '3-4');

    const rec = await request(app).post('/api/auth/recover').send({
      display_name: NAME, grade_group: '3-4', icon_key_1: 5, icon_key_2: 9, new_password: 'newpass1',
    });
    expect(rec.status).toBe(200);

    expect((await request(app).post('/api/auth/login')
      .send({ display_name: NAME, grade_group: '3-4', password: 'newpass1' })).status).toBe(200);
    expect((await request(app).post('/api/auth/login')
      .send({ display_name: NAME, grade_group: '3-4', password: 'oldpass' })).status).toBe(401);
  });

  test('wrong icons → 401; swapped icon ORDER → 401 (order matters)', async () => {
    const NAME = name('Ord');
    await request(app).post('/api/auth/register').send({
      display_name: NAME, grade_group: '3-4', password: 'pass123', icon_key_1: 3, icon_key_2: 8,
    });
    await trackByName(NAME, '3-4');

    expect((await request(app).post('/api/auth/recover').send({
      display_name: NAME, grade_group: '3-4', icon_key_1: 1, icon_key_2: 2, new_password: 'nope12',
    })).status).toBe(401);

    expect((await request(app).post('/api/auth/recover').send({
      display_name: NAME, grade_group: '3-4', icon_key_1: 8, icon_key_2: 3, new_password: 'nope12',
    })).status).toBe(401);
  });

  test('unknown account → 404', async () => {
    const res = await request(app).post('/api/auth/recover').send({
      display_name: name('Nobody'), grade_group: '3-4', icon_key_1: 1, icon_key_2: 2, new_password: 'abcdef',
    });
    expect(res.status).toBe(404);
  });

  test('Grade 1-2 recovery reveals the auto-password (before first login)', async () => {
    const NAME = name('Rk');
    const reg = await request(app).post('/api/auth/register').send({
      display_name: NAME, grade_group: '1-2', icon_key_1: 7, icon_key_2: 10,
    });
    await trackByName(NAME, '1-2');
...
describe('FR7 — Session management', () => {
  ...
  test('logout destroys the session', async () => {
    ...
    await agent.post('/api/auth/register').send({
      display_name: NAME, grade_group: '3-4', password: 'pass123', icon_key_1: 6, icon_key_2: 12,
    });
    await trackByName(NAME, '3-4');
